...
try:
    ...
    if not feedback_email:
        msg = "FEEDBACK_MAIL not set in environment variables"
...
def render_export_theme(config_toml_template_path: str):
    ...
    config_toml, theme_updates, theme_sidebar_updates = set_config_toml(
        config_toml_template_path, updated_themes
    )
    ...
    with cont:
        with st_yled.container(
            key="export-theme-info-box", background_color="#F6F6F6", padding="16px"
        ):
            st_yled.markdown(
                "1. Make sure your project has a `.streamlit` directory",
                key="export-theme-step-1-markdown",
            )
            st_yled.markdown(
                "2. Download and place the `config.toml` file inside the `.streamlit` directory",
                key="export-theme-step-2-markdown",
            )
        ...
        if not theme_updates and not theme_sidebar_updates:
            st_yled.info(
                "**:material/notifications: No theme updates found - yet**\n\nUse the theme editor to make your first changes"
            )

        else:
            with st_yled.expander(
                "I already have a config.toml file",
                ...
                sidebar_theme_code = format_theme_toml(
                    theme_sidebar_updates, "theme.sidebar"
                )

                if theme_updates and theme_sidebar_updates:
                    theme_tab, sidebar_tab = st_yled.tabs(
                        ["[theme]", "[theme.sidebar]"],
                        key="export-theme-updated-theme-tabs",
                    )
                    ...
                    theme_tab = st_yled.tabs(
                        ["[theme]"], key="export-theme-updated-theme-tabs"
                    )
                    with theme_tab[0]:
                        st.code(main_theme_code)

                else:
                    sidebar_tab = st_yled.tabs(
                        ["[theme.sidebar]"], key="export-theme-updated-theme-tabs"
                    )
                    with sidebar_tab[0]:
                        st.code(sidebar_theme_code)
        ...
        bgroup_cont = st.container(
            key="export-theme-button-group-container", horizontal=True
        )

        if bgroup_cont.download_button(
...
def render_export_elements():
    ...
    for element_name in export_elements.keys():
        ...
        for css_prop_format in export_elements[element_name]:
            ...
            for css_selector in css_format.keys():
                ...
                export_css[css_selector][css_prop] = export_elements[element_name][
                    css_prop_format
                ]
    ...
    with cont:
        with st_yled.container(
            key="export-elements-info-box", background_color="#F6F6F6", padding="16px"
        ):
            st_yled.markdown(
                "1. Make sure your project has a `.streamlit` directory",
                key="export-elements-step-1-markdown",
            )

            st_yled.markdown(
                "2. Download and place the `st-styled.css` file inside the `.streamlit` directory",
                key="export-elements-step-2-markdown",
            )

            st_yled.markdown(
                ":material/notification_important: Make sure to run `st_yled.init()` on each app page",
                key="export-elements-note",
            )
        ...
        if not export_css_format:
            st_yled.info(
                "**:material/notifications: No element updates found - yet**\n\nUse the element editor to create first styling",
                key="export-elements-no-updates-info",
            )

        else:
            with st_yled.expander(
                "I already have a st-styled.css file",
                border_style="none",
                background_color=uiconfig.SECONDARY_BACKGROUND_COLOR_DEFAULT,
                ...
                st.write("")
                st.markdown(
                    "Copy and add/replace sections in your `st-styled.css` file"
                )
        ...
        if excluded_elements_message != "":
            st_yled.markdown(
                f":material/notification_important: {excluded_elements_message}",
        ...
        bgroup_cont = st.container(
            key="export-elements-button-group-container", horizontal=True
        )

        if bgroup_cont.download_button(
            "Download st-styled.css",
            data=export_css_format,
            file_name="st-styled.css",
            mime="text/plain",
            type="primary",
        ):
            st.rerun()
...
def render_help_dialog():
    ...
    with st_yled.container(
        key="help-dialog-intro-container",
        horizontal=True,
        ...
        st_yled.markdown(
            "Welcome to st_yled studio - your place for beautiful Streamlit apps",
            key="help-dialog-welcome-markdown",
        )
    ...
    with st_yled.expander(
        ...
        with st_yled.container(
            key="help-dialog-theme-expander-content",
    ...
    with st_yled.expander(
        ...
        with st_yled.container(
            key="help-dialog-elements-expander-content",
            background_color="#f7f8fa",
            padding="16px",
        ):
            st_yled.markdown(
    ...
    with st_yled.expander(
        ...
        with st_yled.container(
            key="help-dialog-new-components-expander-content",
            background_color="#f7f8fa",
            padding="16px",
        ):
            st_yled.markdown(
...
""",
    ...
    with st_yled.container(key="feedback-dialog-form-container"):
        email = st_yled.text_input(
            "Your email (optional)", width=512, key="feedback-email-input"
        )

        feedback = st_yled.text_area(
            "Your feedback, ideas or suggestions", width=512, key="feedback-text-area"
        )

        st.write("")

        bgroup_cont = st_yled.container(
            key="send-feedback-button-group-container", horizontal=True
        )

        if bgroup_cont.button(
            "Send", icon=":material/send:", type="primary", key="send-feedback-button"
        ):
            if feedback.strip() == "":
                st_yled.warning(
                    "Please provide feedback before sending",
                    width=512,
                    key="feedback-empty-warning",
                )
            else:
                ...
                if ses_client:
                    ...
                    ses_client.send_email(
                        Source=feedback_email, Destination={"ToAddresses": [feedback_email]}, Message={
...
components_detail = st.Page(
    "pages/component_detail.py", title="Component Detail", url_path="component-detail"
)
...
with st.sidebar.container(key="sidebar-footer-container"):
    st_yled.markdown(
        ...
        font_size="12px",
        color="#31333F99",
        key="sidebar-footer-getting-started-markdown",
    )

    st_yled.markdown(
        "[st_yled Python Package](https://st-styled.evo-byte.com/)",
        font_size="14px",
        key="sidebar-footer-styled-pkg-link-markdown",
    )
    ...
    st_yled.markdown(
        ...
        font_size="12px",
        color="#31333F99",
        key="sidebar-footer-help-markdown",
    )

    st_yled.markdown(
        "[st_yled Community](https://github.com/EvobyteDigitalBiology/st-styled/discussions)",
        font_size="14px",
        key="sidebar-footer-community-link-markdown",
    )
    st.write("")
...
with sticky_header:
    ...
    with st.container(
        ...
        st_yled.button(
            ...
            key="help-button",
            type="secondary",
            on_click=render_help_dialog,
            border_style="none",
            background_color="#97a6c326",
        )
        ...
        st_yled.button(
            ...
            key="feedback-button",
            type="secondary",
            on_click=render_feedback_dialog,
            border_style="none",
            background_color="#97a6c326",
        )
